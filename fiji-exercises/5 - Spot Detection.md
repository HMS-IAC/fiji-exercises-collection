...
2. open the [beads_001.tif](images/spot_detection/beads_001.tif) image in folder `spot_detection`(drag & drop, or `File > Open`)
...
8. save the ROI to file (`More > Save`)
...
1. open the [beads_001.tif](images/spot_detection/beads_001.tif) image in folder `spot_detection`(drag & drop, or `File > Open`)
...
6. add you detected maxima to the `ROI Manager` and save to file
    ...
    - add the points to the `ROI Manager` (e.g. press `t` on your keyboard)
        - NOTE: only one ROI will be added, it contains all the points.
    - save the ROI to file (`More > Save`).
7. load saved ROIs from the manual detection exercise ([5.1](#manual-spot-detection-with-the-multi-point-tool)) and compare results
